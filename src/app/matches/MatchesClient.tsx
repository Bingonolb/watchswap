  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      {rows.map(m => {
        ...
        const theirs = m.watch_a.owner_id === userId ? m.watch_b : m.watch_a;
        ...
        return (
          <Link key={m.id} href={`/messages/${m.id}`} style={{ textDecoration: "none" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 16, background: "#fff", borderRadius: 16, border: "1px solid #e8e8e8", padding: "14px 18px", boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: 15, fontWeight: 700, color: "#111", marginBottom: 2 }}>{theirs.brand} {theirs.model}</p>
