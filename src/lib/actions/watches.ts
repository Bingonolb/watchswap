...
export async function updateWatch(
  ...
  if (!brand || !model || !condition) return { error: "Marque, modèle et état sont requis." };

  // Get existing photos to keep
  const { data: existing } = await supabase.from("watches").select("photos").eq("id", watchId).eq("owner_id", user.id).maybeSingle();
  if (!existing) return { error: "Montre introuvable." };

  const photoUrls: string[] = [...(existing.photos ?? [])];

  for (const file of files.slice(0, Math.max(0, 5 - photoUrls.length))) {
    const ext = file.name.split(".").pop() || "jpg";
    const path = `${user.id}/${watchId}-${randomUUID()}.${ext}`;
    const { error: uploadError } = await supabase.storage.from("watch-photos").upload(path, file, { contentType: file.type });
    if (uploadError) return { error: `Upload photo échoué : ${uploadError.message}` };
    const { data: pub } = supabase.storage.from("watch-photos").getPublicUrl(path);
    photoUrls.push(pub.publicUrl);
  }
  ...
  const { error } = await supabase.from("watches").update({
    ...
    has_box: hasBox,
    has_papers: hasPapers,
  }).eq("id", watchId).eq("owner_id", user.id);
